...
const requirements = [
  ...
  requirement("AUTH-001", "Owner Console versus customer dashboard authorization", "Owner-only operational APIs are separate from customer guild workspaces. Customer users can view only Discord guilds where they have Manage Guild or Administrator.", {
    ...
    testRequirements: [...commonTest, "two-user/two-guild authorization test"], sourceStatus: "LOCAL VERIFIED", localTestStatus: "LOCAL VERIFIED", evidencePath: ["test/paradiseFoundation.test.js"], nextExactAction: "Integrate workspace access into customer-facing server selector/config APIs after the existing owner-only endpoint split is mapped.", affectedFiles: ["src/paradiseGuildScope.js", "src/server.js", "src/adminAuth.js"], affectedDatabaseTables: ["users", "oauth_links", "audit_logs"], securityRisk: "critical"
  }),
  ...
  requirement("CFG-001", "Configuration versioning and audit foundation", "Every guild configuration save creates a schema-versioned snapshot, field-level diff, actor, source and rollback record.", {
    ...
    testRequirements: [...commonTest, "save/reload/audit test"], sourceStatus: "LOCAL VERIFIED", localTestStatus: "LOCAL VERIFIED", evidencePath: ["test/paradiseFoundation.test.js", "test/paradise-dashboard.test.js"], nextExactAction: "Add authorized config-version history reading and rollback preview before exposing rollback actions.", affectedFiles: ["src/paradiseConfigVersioning.js", "src/server.js", "src/paradise3a59.js"], affectedDatabaseTables: ["settings", "audit_logs"], securityRisk: "high"
  }),
  ...
  requirement("DB-001", "PostgreSQL guild-scope data foundation", "Guild-local Paradise state moves from opaque state JSON toward scoped PostgreSQL records while global identity remains separate.", {
    ...
    testRequirements: [...commonTest, "Prisma schema validation", "guild isolation test"], sourceStatus: "SOURCE ONLY", nextExactAction: "Inventory existing Setting JSON state and prepare additive migration preview; do not migrate production data yet.", affectedFiles: ["prisma/schema.prisma", "src/paradise3a59.js"], affectedDatabaseTables: ["settings", "audit_logs"], securityRisk: "critical"
  }),
  ...
  requirement("BACKUP-001", "Backup integrity foundation", "Backups carry schema version, counts, overwrite and panel metadata, checksum, validation and restore dry-run status.", {
    ...
    testRequirements: [...commonTest, "backup checksum test", "restore dry-run test"], sourceStatus: "LOCAL VERIFIED", localTestStatus: "LOCAL VERIFIED", evidencePath: ["test/paradiseBackupIntegrity.test.js"], nextExactAction: "Add restore dry-run validation and encrypted storage policy before any destructive rebuild path can consume backups.", affectedFiles: ["src/paradiseBackupIntegrity.js", "src/paradise3a59.js", "src/discordBot.js", "src/server.js"], affectedDatabaseTables: ["settings", "audit_logs"], securityRisk: "critical"
  }),
  ...
  requirement("CMD-001", "Central command registry", "Every command/subcommand has one manifest for template, module, premium tier, channel, role, parameters, help text, audit and registration scope.", {
    ...
    testRequirements: [...commonTest, "template command visibility test", "role visibility test", "runtime denial test"], sourceStatus: "LOCAL VERIFIED", localTestStatus: "LOCAL VERIFIED", evidencePath: ["test/paradiseFoundation.test.js"], nextExactAction: "Map existing slash command builders, help rendering and handlers to this registry incrementally without unregistering commands prematurely.", affectedFiles: ["src/paradiseCommandRegistry.js", "src/paradise3a59.js", "src/discordBot.js"], securityRisk: "high"
  }),
  ...
  requirement("COMP-001", "Persistent Discord component protocol", "Versioned custom IDs are guild scoped, restart safe, authorization checked and repairable without collectors.", {
    ...
    testRequirements: [...commonTest, "restart interaction test", "stale component test", "cross-guild replay test"], sourceStatus: "LOCAL VERIFIED", localTestStatus: "LOCAL VERIFIED", evidencePath: ["test/paradiseFoundation.test.js"], nextExactAction: "Migrate one non-destructive component family in the test guild and retain old-family repair messaging during the compatibility window.", affectedFiles: ["src/paradiseComponentProtocol.js", "src/paradise3a59.js", "src/discordBot.js"], securityRisk: "high"
  }),
  requirement("SETUP-001", "Exact setup preview, status and audit commands", "Setup commands use test-guild guard, preview/create-missing/repair/repost/backup/restore/rebuild/status/audit behavior and never mutate production without confirmation.", {
    ...
    testRequirements: [...commonTest, "test guild safe preview", "non-allowed guild denial", "idempotency test"], sourceStatus: "LOCAL VERIFIED", localTestStatus: "LOCAL VERIFIED", liveDiscordStatus: "LIVE DISCORD VERIFIED", evidencePath: ["artifacts/post-security-backlog/3a71-compact-turkish-test-lab-live-proof.json"], nextExactAction: "Rebase existing setup handlers on the registry and environment guard; preserve current test guild behavior.", affectedFiles: ["src/paradise3a59.js", "src/discordBot.js", "src/server.js"], affectedDiscordObjects: ["test-guild structure"], securityRisk: "critical"
  }),
  ...
  requirement("TEST-001", "Test-guild-only smoke proof", "The exact test guild proves safe guard, setup preview/status/audit, idempotency, persistent component restart and RBAC denial without production mutation.", {
    ...
    testRequirements: ["live bot test on guild " + testGuild, "restart proof", "guard negative proof"], sourceStatus: "LOCAL VERIFIED", liveDiscordStatus: "LIVE DISCORD VERIFIED", evidencePath: ["artifacts/post-security-backlog/3a71-compact-turkish-test-lab-live-proof.json"], nextExactAction: "Repeat only after Milestone 1 guard/component/RBAC changes; do not rebuild the lab for artifact-only work.", affectedFiles: ["src/paradise3a59.js", "src/discordBot.js"], affectedDiscordObjects: ["test guild only"], securityRisk: "high"
  }),
  requirement("TPL-001", "Compact template architecture", "Community, Clan and TSBTR enforce channel budgets, disabled-module absence, same-channel consolidation and test-before-main installation.", { module: "templates", priority: "P1", dependencies: ["SETUP-001", "RBAC-001"], acceptanceCriteria: ["Community 10-12 public/4-6 private", "Clan 15-18 public/6-8 private", "TSBTR 14-17 public/6-8 private", "no default role-guide"], testRequirements: [...commonTest, "three template install comparison"], sourceStatus: "SOURCE ONLY", nextExactAction: "Defer until Milestone 1 setup foundation is stable.", affectedFiles: ["src/paradise3a59.js"], securityRisk: "medium" }),
  ...
  requirement("SESSION-001", "Training, tryout, results and activity", "Active posts are plain Markdown in the guild language; result authority, evidence, activity quotas, promotion and lifecycle replies are controlled and audited.", { module: "sessions", priority: "P1", dependencies: ["RBAC-001", "COMP-001", "PROFILE-001"], acceptanceCriteria: ["no active embed", "reply lifecycle", "authority ceiling", "training activity review"], testRequirements: [...commonTest, "training/tryout live lifecycle", "authority persona test"], sourceStatus: "LOCAL VERIFIED", liveDiscordStatus: "LIVE DISCORD VERIFIED", evidencePath: ["artifacts/post-security-backlog/3a71-compact-turkish-test-lab-live-proof.json"], nextExactAction: "Re-test only after component/RBAC upgrade; localize remaining English labels later.", securityRisk: "high" }),
  requirement("TICKET-001", "Ticket state machine and transcript-first delete", "Open/claimed/closed controls are state-aware. Close/reopen/delete, transcripts, retention, naming, logs and safe failure behavior use one backend state machine.", { module: "tickets", priority: "P1", dependencies: ["DB-001", "RBAC-001", "COMP-001", "BACKUP-001"], securityCritical: true, acceptanceCriteria: ["closed shows Reopen/Delete only", "delete creates transcript first", "failure blocks deletion", "staff-only transcript access"], testRequirements: [...commonTest, "close/reopen/delete/transcript failure live test"], sourceStatus: "SOURCE ONLY", nextExactAction: "Defer until persisted state/component protocol are in place.", securityRisk: "critical" }),
