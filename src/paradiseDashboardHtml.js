...
<head>
  ...
  <style>
    :root{--brand:#9b5cff;--brand-soft:#9b5cff33}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 20% 0,var(--brand-soft),transparent 34%),#08050d;color:#f8f3ff;font:15px Inter,Segoe UI,sans-serif}
...
<main>
  ...
  <section class="grid">
    ...
    <article class="card"><h2>Embed appearance</h2><p class="muted">Controls the colored stripe on Paradise embeds. Use a six-digit HEX color.</p><div class="swatch"></div><div class="color-row"><input id="brandPicker" type="color" value="#9b5cff" aria-label="Brand color"><input id="brandHex" maxlength="7" value="#9B5CFF" spellcheck="false"></div><button data-save="branding">Save color</button></article>
...
async function load(){
  ...
  const brand=/^#[0-9a-f]{6}$/i.test(j.config.brandColor||'')?j.config.brandColor.toUpperCase():'#9B5CFF';
